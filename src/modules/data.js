...
export const names = {
    'aifml': 'Будущие лидеры рынка',
    'aifot': 'Будущее путешествий',
    'aifob': 'Будущее блокчейна',
    'aifoh': 'Будущее развлечений',
    'aifoi': 'Будущее интернета',
    'aifoc': 'Будущее потребления',
    'aifoe': 'Будущее здоровья',
};

export const getName = (type) => {
    type = type.toLowerCase();

    return names[type] ?? 'Динамика портфеля';
}
...
export default async (type) => {
    ...
    const items = json
        .filter(i => itemNames.includes(i[""].toLowerCase()))
        .reverse()
        .map((item, index) => {
